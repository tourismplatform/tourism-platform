          ...
          {mounted && (
            isAuthenticated ? (
              <div
                ...
                style={{ width: 38, height: 38, background: 'linear-gradient(135deg, #1a4fd6, #ff5722)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer' }}
              >
        ...
        <div style={{ position: 'fixed', top: 64, left: 0, width: '100%', height: 'calc(100vh - 64px)', background: 'white', zIndex: 99, padding: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {mounted && isAuthenticated ? (
            <>
              ...
              <Link href="/profile" onClick={() => setIsOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px', borderRadius: 12, background: '#eff6ff', color: '#1a4fd6', textDecoration: 'none', fontWeight: 600 }}><UserIcon size={20} /> Mon Profil ({user?.name})</Link>
            </>
          ) : (
