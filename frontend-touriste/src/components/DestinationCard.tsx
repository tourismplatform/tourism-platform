...
export default function DestinationCard({ destination, onClick }: Props) {
  ...
  return (
    <div onClick={onClick} style={{
      background: 'var(--white)', borderRadius: 14, boxShadow: '0 4px 24px rgba(10,15,30,0.08)',
    ...
    >
      ...
      <div style={{ height: 200, background: color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '4rem', position: 'relative' }}>
        ...
        {/* Badge catégorie */}
        <span style={{ position: 'absolute', top: 12, left: 12, background: 'var(--accent)', color: 'white', fontSize: '0.7rem', fontWeight: 700, padding: '4px 10px', borderRadius: 20, letterSpacing: '0.5px' }}>
          {t(`cat_${destination.category.toLowerCase() as any}` as TranslationKey) || destination.category}
        </span>
        {/* Bouton favori */}
        <button
          onClick={e => { e.stopPropagation(); setFav(!fav); }}
          style={{ position: 'absolute', top: 10, right: 10, width: 32, height: 32, background: 'var(--white)', opacity: 0.9, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer', fontSize: '1rem', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
          {fav ? '❤️' : '🤍'}
        </button>
      </div>
      ...
      <div style={{ padding: 16 }}>
        <div style={{ fontWeight: 700, fontSize: '1rem', marginBottom: 4, color: 'var(--dark)' }}>
          {destination.name}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontWeight: 700, color: 'var(--primary)', fontSize: '0.95rem' }}>
           {formatPrice(destination.price || destination.price_per_person || 0)} <span style={{ fontSize: '0.75rem', color: 'var(--gray)', fontWeight: 400 }}>/pers.</span>
          </span>
          <span style={{ background: 'rgba(217, 119, 6, 0.1)', color: '#d97706', padding: '3px 8px', borderRadius: 6, fontSize: '0.78rem', fontWeight: 700 }}>
