...
function DestinationsList() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 64px)' }}>
      ...
      <div className="mobile-only" style={{ padding: '16px 20px', background: 'white', borderBottom: '1px solid #e5e7eb' }}>
        <button 
          onClick={() => setShowFilters(!showFilters)}
          style={{ width: '100%', background: '#eff6ff', color: '#1a4fd6', border: '1.5px solid #bfdbfe', padding: '12px', borderRadius: 10, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
        >
          {showFilters ? 'Fermer les filtres ✕' : 'Filtrer les destinations 🔍'}
        </button>
      </div>
      ...
      <div style={{ display: 'flex', flex: 1, flexDirection: 'row' }} className="flex-col md:flex-row">
        {/* SIDEBAR FILTRES */}
        <aside style={{ 
          background: 'white', borderRight: '1px solid #e5e7eb', padding: 24, 
        ...
        {/* CONTENU PRINCIPAL */}
        <main style={{ padding: 'clamp(16px, 4vw, 24px)', background: '#f4f6fa', flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, background: 'white', padding: '12px 16px', borderRadius: 10, flexWrap: 'wrap', gap: 10 }}>
